...
Агенты:
- `landing-editor` — тексты одной ветки, без правок каркаса
- `media-importer` — раскладывает загруженные кадры по блокам: смотрит каждое
  изображение, выбирает hero и og, собирает галерею, группирует планировки
  по типам из `units`, пишет alt
