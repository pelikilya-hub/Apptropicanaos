...
`config.media`. Порядок работы с файлами из sales kit:

```bash
npm run optimize:media -- --slug <slug>              # сжать исходники
npm run import:media  -- --slug <slug> --write       # собрать черновик media
```

Из китов приходят исходники по 5–12 МБ — на мобильном интернете такая страница
не открывается. Оптимизатор переводит их в WebP с ограничением по ширине
(по умолчанию 2400px, качество 82) и заменяет исходники: оригиналы остаются
у застройщика, в репозитории лежат рабочие копии.
